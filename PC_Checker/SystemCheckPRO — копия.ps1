...
<#
    ...
    - Проверка кода через Netlify Function (кроме мастер-ключа ADMIN123) с получением projectId
    - Загрузка требований проекта по projectId и сравнение с собранными данными
    - ОС теперь сравнивается как минимальное требование (Windows 10 подходит для Windows 11 и выше)
    - Ошибки отображаются прямо в окне приветствия (красный текст)
    - Кнопка "Закрыть" в левом нижнем углу
    - Отправка результатов на сервер через .NET HttpClient
    - Реальный тест скорости интернета до указанных серверов
    - Автоматический запрос прав администратора через UAC
    - Проверка целостности WAV-файла после записи
    - Анализ аудиоустройств (тип подключения, статус, назначение)
    - Определение типа компьютера (ноутбук/ПК)
    - Определение типа интернет-подключения (LAN/Wi-Fi)
    - Временная кнопка "Пропустить тест" в окне печати
    - Промежуточное окно благодарности без кнопок
    - Финальное окно с детализированным вердиктом (размер как приветственное)
    - Определение активных аудиоустройств (по умолчанию) через Windows API
    - Проверка типа подключения гарнитуры (USB)
#>
...
# ===================== ФУНКЦИЯ АНАЛИЗА АУДИОУСТРОЙСТВ =====================
...
# ===================== ФУНКЦИЯ ОПРЕДЕЛЕНИЯ АКТИВНЫХ АУДИОУСТРОЙСТВ (WINDOWS API) =====================
...
public class AudioDevice
{
    ...
    private static readonly Guid PKEY_Device_FriendlyName = new Guid("a45c254e-df1c-4efd-8020-67d146a8500e");
    private const uint PKEY_Device_FriendlyName_pid = 14;

    private static string GetDefaultAudioEndpointName(EDataFlow flow)
    {
        try
        {
            ...
            IMMDevice device;
            int hr = devEnum.GetDefaultAudioEndpoint(flow, ERole.eConsole, out device);
            if (hr != 0) return null;
            IPropertyStore store;
            device.OpenPropertyStore(STGM_READ, out store);
            PropertyKey key = new PropertyKey { fmtid = PKEY_Device_FriendlyName, pid = PKEY_Device_FriendlyName_pid };
            PropVariant value;
            store.GetValue(ref key, out value);
            string name = Marshal.PtrToStringUni(value.pszVal);
    ...
    public static string GetDefaultDeviceName(Flow flow)
    {
        return GetDefaultAudioEndpointName(flow == Flow.Render ? EDataFlow.eRender : EDataFlow.eCapture);
    }
}
"@

function Get-DefaultAudioDevices {
    $inputName = [AudioDevice]::GetDefaultDeviceName('Capture')
    $outputName = [AudioDevice]::GetDefaultDeviceName('Render')
